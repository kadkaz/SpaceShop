...
_.extend(CellFormatter.prototype, {
  ...
  /**
     Takes a raw value from a domain and returns an optionally formatted string
  ...
  */
  ...
  /**
     ...
     appropriately typed value for persistence in the domain.

     If the user input is invalid or unable to be converted to a raw value
     suitable for persistence in the domain, toRaw must return `undefined`.
...
var CellEditor = Backgrid.CellEditor = Backbone.View.extend({
  ...
  /**
     ...
     @param {Backbone.Model} options.domain

     @throws {TypeError} If `formatter` is not a formatter instance, or when
     `domain` or `column` are undefined.
  */
  initialize: function (options) {
    Backgrid.requireOptions(options, ["formatter", "column", "domain"]);
...
var InputCellEditor = Backgrid.InputCellEditor = CellEditor.extend({
  ...
  */
  ...
  /**
     ...
     in the editor to a raw value for saving into the domain using the formatter.
     ...
     Triggers a Backbone `backgrid:edited` event from the domain when successful,
...
/**
   ...
   table cell with the domain value converted to a string using the
...
var Cell = Backgrid.Cell = Backbone.View.extend({
  ...
  */
  initialize: function (options) {
    Backgrid.requireOptions(options, ["domain", "column"]);
    this.column = options.column;
    if (!(this.column instanceof Column)) {
      this.column = new Column(this.column);
    }
  ...
  /**
     ...
     domain's raw value for this cell's column.
  */
  render: function () {
    this.$el.empty();
  ...
  /**
     ...
     This method triggers a Backbone `backgrid:edit` event from the domain when
     ...
     `backgrid:editing` event is triggered from the domain. The cell and the
     constructed cell instance are also sent as parameters in the event.

     When the domain triggers a `backgrid:error` event, it means the editor is
     unable to convert the current user input to an apprpriate value for the
     domain's column, and an `error` CSS class is added to the cell accordingly.
  */
...
var BooleanCellEditor = Backgrid.BooleanCellEditor = CellEditor.extend({
  ...
  /**
     Renders a checkbox and check it if the domain value of this column is true,
     uncheck otherwise.
  */
  render: function () {
  ...
  */
  ...
  /**
     Event handler. Save the value into the domain if the event is `change` or
...
/**
   ...
   checkbox is checked if the domain value is true, unchecked otherwise.
...
var SelectCellEditor = Backgrid.SelectCellEditor = CellEditor.extend({
  ...
  */
  ...
  /**
     Saves the value of the selected option to the domain attribute. Triggers a
     `backgrid:edited` Backbone event from the domain.
  */
  save: function (e) {
    var model = this.model;
    var column = this.column;
  ...
  /**
     Triggers a `backgrid:edited` event from the domain so the body can close
     this editor.
  */
  close: function (e) {
    var model = this.model;
...
/**
   ...
   During display mode, the default formatter will normalize the raw domain value
   to an array of values whether the raw domain value is a scalar or an
   ...
   conversion is performed when saving into the domain. This behavior is not
...
var Column = Backgrid.Column = Backbone.Model.extend({
  ...
  /**
     ...
     @param {string} attrs.name The name of the domain attribute.
     ...
     formatter to use to convert between raw domain values and user input.
...
var Columns = Backgrid.Columns = Backbone.Collection.extend({
  ...
  /**
     @property {Backgrid.Column} domain
   */
  model: Column
});
/*
...
/**
   Row is a simple container view that takes a domain instance and a list of
   column metadata describing how each of the domain's attribute is to be
...
var Row = Backgrid.Row = Backbone.View.extend({
  ...
  requiredOptions: ["columns", "domain"],
  ...
  /**
     ...
     @param {Backbone.Model} options.domain The domain instance to render.

     @throws {TypeError} If options.columns or options.domain is undefined.
  */
  initialize: function (options) {
  ...
  */
  ...
  /**
     Renders a row of cells for this row's domain.
  */
  render: function () {
    this.$el.empty();
...
var Body = Backgrid.Body = Backbone.View.extend({
  ...
  */
  ...
  /**
     ...
     When called directly, it accepts a domain or an array of models and an
     option hash just like
     [Backbone.Collection#add](http://backbonejs.org/#Collection-add) and
     delegates to it. Once the domain is added, a new row is inserted into the
     body and automatically rendered.

     When called as a callback of an `add` event, splices a new row into the
     body and renders it.

     @param {Backbone.Model} model The domain to render as a row.
  ...
  */
  ...
  /**
     ...
     delegates to it. Once the domain is removed, a corresponding row is removed
     from the body.

     When called as a callback of a `remove` event, splices into the rows and
     removes the row responsible for rendering the domain.

     @param {Backbone.Model} model The domain to remove from the body.
     @param {Backbone.Collection} collection When called directly, this
     parameter is actually the options to
     [Backbone.Collection#remove](http://backbonejs.org/#Collection-remove).
     @param {Object} options When called directly, this must be null.
  ...
  */
  ...
  /**
     ...
     @param {Backbone.Model} model The originating domain
     @param {Backgrid.Column} column The originating domain column
...
/**
   ...
   By default, a Grid treats each domain in a collection as a row, and each
   attribute in a domain as a column. To render a grid you must provide a list of
...
var Grid = Backgrid.Grid = Backbone.View.extend({
  ...
  /**
     ...
     @param {Object} options
     @param {Backbone.Collection.<Backgrid.Column>|Array.<Backgrid.Column>|Array.<Object>} options.columns Column metadata.
     @param {Backbone.Collection} options.collection The collection of tabular domain data to display.
